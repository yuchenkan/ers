...
#define m4_ns(atomic_load)(m) \
  ({ typeof (m) _m = m;	typeof (*_m) _v;				\
     _eri_atomic_foreach_size (_m, m4_ns(atomic_load, _), _v); _v; })
#define m4_ns(atomic_store)(m, v) \
  do { typeof (m) _m = m; typeof (*_m) _v = (typeof (*_m)) v;		\
       _eri_atomic_foreach_size (_m, m4_ns(atomic_store, _), _v); } while (0)
...
#define m4_ns(atomic_inc_dec, _)(m, cinc, inc) \
  do {									\
    typeof (m) _m = m;							\
    _eri_atomic_foreach_size (_m, m4_ns(atomic_inc_dec, __),		\
			      cinc, inc);				\
  } while (0)
...
#define m4_ns(atomic_add_fetch)(m, v) \
  ({ typeof (m) _m = m;							\
     ...
     typeof (*_m) _r = _v;						\
     _eri_atomic_foreach_size (_m, m4_ns(atomic_xadd, _), _v);		\
     (typeof (*_m)) (_r + _v); })
...
#define m4_ns(atomic_and)(m, v) \
  do {									\
    typeof (m) _m = m;							\
    typeof (*_m) _v = (typeof (*_m)) v;					\
    _eri_atomic_foreach_size (_m, m4_ns(atomic_and, _), _v);		\
  } while (0)
...
#define m4_ns(atomic_bit_test_set)(m, off) \
  ({ typeof (m) _m = m;							\
     typeof (off) _off = off;						\
     uint8_t _r;							\
     _eri_atomic_foreach_size1 (_m, m4_ns(atomic_bts, _), _off, _r);	\
     _r; })
...
#define m4_ns(atomic_exchange)(m, v) \
  ({ typeof (m) _m = m;							\
     ...
     _eri_atomic_foreach_size (_m, m4_ns(atomic_xchg, _), _r);		\
     _r; })

#define m4_ns(atomic_cmpxchg, _)(sz, _m, _r, _e, _d) \
  asm volatile (ERI_STR (m4_ns(atomic_cmpxchg, __) (1, sz,		\
				%_ERI_ASM_TEMPLATE_SIZE (sz, 3), %1))	\
		:  "=@ccz" (_r), "+m" (*_m) : "a" (_e), "r" (_d))
...
#define m4_ns(atomic_compare_exchange)(m, e, d) \
  ({ typeof (m) _m = m;							\
     ...
     uint8_t _r;							\
     _eri_atomic_foreach_size (_m, m4_ns(atomic_cmpxchg, _),		\
			       _r, _e, _d);				\
     _r; })

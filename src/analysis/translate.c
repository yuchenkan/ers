...
ir_reg_idx_from_xed (xed_reg_enum_t reg)
{
  ...
  eri_lassert (reg_idx != TRANS_REG_NUM);
  return reg_idx;
}
...
static uint8_t
ir_encode_load_imm (eri_file_t log, uint8_t *bytes, uint8_t dst, int64_t src)
{
...
static void
ir_update_trace_guest (struct ir_trans *tr, uint8_t idx,
		       struct trans_loc loc)
{
...
static void
ir_try_fix_guest_loc (struct ir_trans *tr, uint8_t idx,
		      struct ir_guest_loc *guest_loc)
{
  ...
  if ((trace->tag == TRANS_LOC_REG
	  ...
	  && (guest->node || guest->imm != trace->val)))
    {
      ...
      ir_update_trace_guest (tr, idx, *trace);
    }
}
			 ...
			 struct ir_def *def)
{
  ...
  for (i = 0; i < TRANS_REG_NUM; ++i)
    if (flat->guest_locs[i].def == def)
      ir_try_fix_guest_loc (tr, i, flat->guest_locs + i);
}
...
{
  if (def) ir_trace_access (tr, def->node, len);
}
  ...
  for (inst_reg = node->inst.regs; inst_reg->op; ++inst_reg)
    {
      ...
      ir_append_ra (ras, ir_inst_designated_reg (dec, op)
			? ir_reg_idx_from_dec_op (dec, op) : TRANS_REG_NUM,
...
static void
ir_assign_emit_inst (struct ir_trans *tr, struct ir_node *node,
		     struct ir_ra *a)
{
  xed_decoded_inst_t *dec = &node->inst.dec;
  xed_operand_values_t *ops = xed_decoded_inst_operands (dec);
		    ...
		    struct ir_node *node)
{
  ...
  for (i = 0; i < TRANS_REG_NUM; ++i)
    if (! regs[i].def->node)
      trans_set_loc (tr->final_locs + i, TRANS_LOC_IMM, regs[i].def->imm);
    else
...
static void
ir_assign_emit_store (struct ir_trans *tr,
		      struct ir_node *node, struct ir_ra *a)
{
  struct ir_enc_mem_args dst;
  ...
  ir_trace_access (tr, node->store.dst.regs.write.def->node, len);
}
		     ...
		     struct ir_node *node, struct ir_ra *a)
{
  ...
  ir_trace_access (tr, node->load.src.regs.read.def->node, len);
}
...
static void
ir_assign_emit_add (struct ir_trans *tr,
		    struct ir_node *node, struct ir_ra *a)
{
		...
		struct ir_node *node, struct ir_ra *ras, uint32_t n)
{
  switch (node->tag)
    {
    case IR_INST: ir_assign_emit_inst (tr, node, ras); break;
    case IR_END:
    case IR_ERR_END:
    ...
    case IR_STORE: ir_assign_emit_store (tr, node, ras); break;
    case IR_LOAD: ir_assign_emit_load (tr, node, ras); break;
    case IR_ADD: ir_assign_emit_add (tr, node, ras); break;
    case IR_COND_STR_OP:
...
ir_generate (struct ir_dag *dag, void *analysis)
{
  ...
  ir_init_trace_accesses (&tr.trace_reads, dag->pool, flat.read_num);
  ir_init_trace_accesses (&tr.trace_writes, dag->pool, flat.write_num);
  ...
  eri_lassert (tr.trace_reads.i == flat.read_num);
  eri_lassert (tr.trace_writes.i == flat.write_num);
...
eri_trans_create_active (struct eri_trans_create_active_args *args)
{
  ...
  uint64_t *cond_reads = act->local + LOCAL_PREDEFINED_NUM + tr->reads.num;
  eri_memset (cond_reads, 0, tr->reads.cond_count);
  uint64_t *cond_writes = cond_reads
		+ eri_div_ceil (tr->reads.cond_count, 8) + tr->writes.num;
  eri_memset (cond_writes, 0, tr->writes.cond_count);

  uint64_t *l = act->local + tr->static_local_num;
		      ...
		      struct eri_mcontext *mctx, struct eri_registers *regs)
{
  ...
  for (i = 0; i < TRANS_REG_NUM; ++i)
    if (i != TRANS_RIP)
      trans_set_loc (locs + i, TRANS_LOC_LOCAL, tr->static_local_num + i);
    else

...
{									\
  return pfx##_rbt_parent (n)						\
	   ? pfx##_rbt_parent (pfx##_rbt_parent (n)) : 0;		\
}									\
									\
static inline node_type *						\
...
{									\
  ...
  node->pfx##_rbt_parent						\
		 = node->pfx##_rbt_left = node->pfx##_rbt_right = 0;	\
									\
...
{									\
  node_type *c = ! n->pfx##_rbt_right					\
			? n->pfx##_rbt_left : n->pfx##_rbt_right;	\
									\
...
{									\
  ...
  _RBT_CHECK (pfx##_rbt_check (tree));					\
									...
									\
attr node_type *							\
pfx##_rbt_get (tree_type *tree, key_type *key, int32_t flags)		\
{									\
...
{									\
  return tree->pfx##_rbt_root						\
		? pfx##_rbt_get_min (tree->pfx##_rbt_root) : 0;		\
}									\
									\
attr node_type *							\
pfx##_rbt_get_next (node_type *node)					\
{									\
  if (node->pfx##_rbt_right)						\
    return pfx##_rbt_get_min (node->pfx##_rbt_right);			\
									\
  node_type *n = node;							\
  while (pfx##_rbt_parent (n)						\
	 && n == pfx##_rbt_parent (n)->pfx##_rbt_right)			\
    n = pfx##_rbt_parent (n);						\
  return pfx##_rbt_parent (n);						\
}									\
									\
...
#define ERI_RBT_FOREACH(pfx, tree, iter) \
  for (iter = pfx##_rbt_get_first (tree);				\
	 iter; iter = pfx##_rbt_get_next (iter))

...
let live = [ 'rtld', 'start', 'clone', 'clear-tid', 'raise', 'sig-ignore', 'sig-nest',
	     'sig-exit-group', 'sig-sig-mask', 'sig-sig-prepare-sync',
	     'sigaltstack', 'sigprocmask', 'sigpending', 'sigsuspend', 'sigtimedwait', 'signalfd',
	     'sig-mask-async-ut' ];

live = live.concat ([ 'main', 'syscall', 'sync-async', 'atomic', 'atomic-ext', 'sig-action' ].map (x => `sig-hand-${x}-ut`));
